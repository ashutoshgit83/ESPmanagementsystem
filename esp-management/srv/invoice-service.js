...
class InvoiceServiceHandler extends cds.ApplicationService {
  init() {
    ...
    // ================================================================
    // INVOICE LINES - Compute PO navigation URL
    // ================================================================

    this.after('READ', InvoiceLines, async (results, req) => {
      const { PurchaseOrderItems, InvoiceLines: InvLines } = cds.entities('esp.management');
      const items = Array.isArray(results) ? results : [results];
      for (const line of items) {
        if (!line || !line.ID) continue;
        try {
          // Get poItem_ID from DB if not in the result
          let poItemId = line.poItem_ID;
          if (!poItemId) {
            const dbLine = await SELECT.one.from(InvLines).columns('poItem_ID').where({ ID: line.ID });
            poItemId = dbLine ? dbLine.poItem_ID : null;
          }
          if (poItemId) {
            const poItem = await SELECT.one.from(PurchaseOrderItems).columns('purchaseOrder_ID').where({ ID: poItemId });
            if (poItem && poItem.purchaseOrder_ID) {
              line.poNavigationUrl = '/purchase-orders/webapp/index.html#app-display&/PurchaseOrders(ID=' + poItem.purchaseOrder_ID + ',IsActiveEntity=true)';
            }
          }
        } catch(e) { /* skip */ }
      }
